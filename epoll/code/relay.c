...
void relay(int fd1, int fd2)
{
    ...
    while (fsm12.state != STATE_T || fsm21.state != STATE_T)
    {
        ...
        epoll_ctl(epfd,EPOLL_CTL_MOD,fd1,&ev); //对fd1布置任务
        ...
        while(epoll_wait(epfd,&ev,1,-1) < 0) //timeout设置为-1阻塞，直到发生感兴趣的行为为止
        {
            if(errno == EINTR)
            {
                continue;
    ...
    // 关闭epoll实例，因为他是文件描述符，所以用close关闭
    close(epfd);
